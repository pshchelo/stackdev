...
__usage="
Usage: $(basename "$0") [-hv] [-n NAME] -i [IMAGE] [-f FLAVOR]
...
if [ -z "$server_image" ]; then
    # get the image id and name, replace the space betwen them (the fist space in the line) with backslash
    for im in $(openstack image list -f value -c ID -c Name | grep -i cirros | sed '0,/ /{s/ /\\/}'); do
        # split on backslash
        image_id="${im%\\*}"
        image_name="${im#*\\}"
...
echo "$server_id" "$fip" "$server_name"
