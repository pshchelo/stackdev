...
teletype "Create a flavor that consumes as much CPU as possible"
teletype "(one can not create an instance with CPUs more than actual physical cores):"
telerun openstack flavor create maxCPU --vcpus 8 --disk 1 --ram 128
...
telerun openstack server show does-not-fit -f json -c fault | grep message
...
teletype "And wait for resource to be applied:"
...
telerun openstack resource provider set --amend $first_resprov_id --resource VCPU:allocation_ratio=8
...
telerun openstack server --os-compute-api-version 2.37 create now-it-fits --nic none --flavor manyCPU --image Cirros-6.0 --wait
...
# rollback
telerun openstack server delete now-it-fits it-fits
